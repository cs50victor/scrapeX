...
s = requests.Session()
randHeader = choice(userAgentList)
...
randomHeader = {
        ...
        "User-Agent": randHeader,
    }

class MakeRequest():
    def __init__(self, url):
        self.url = url
        self.defaultTimeout=(5,8) # (connect timeout, response waiting time after client has established a connection)

        # gateway = ApiGateway(url, regions=EXTRA_REGIONS, access_key_id="ID", access_key_secret="SECRET")
        # gateway.start()
        # s.mount(url, gateway)
        self.response = s.get(self.url,headers=randomHeader,timeout=self.defaultTimeout, stream=True)
