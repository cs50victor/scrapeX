...
use utils::{Response, ResponseType};
use std::error::Error;
...
async fn index() -> impl Responder {
    ...
    match scrape(url).await {
        Ok(res) => HttpResponse::Ok().json(Response::new(ResponseType::Data(res))),
        Err(err) => HttpResponse::NoContent().json(Response::new(ResponseType::Error(err.to_string())))   
    }
    // HttpResponse::Ok().json(Response::new(ResponseType::Data("rest")))
}
